...
ODriveCanNode::ODriveCanNode(const std::string & node_name) : rclcpp::Node(node_name)
{
  ...
  ctrl_publisher_ =
    rclcpp::Node::create_publisher<ControllerStatus>("controller_status", ctrl_stat_qos);
  ...
  subscriber_ = rclcpp::Node::create_subscription<ControlMessage>(
    "control_message", ctrl_msg_qos, std::bind(&ODriveCanNode::subscriber_callback, this, _1));

  rclcpp::QoS srv_qos(rclcpp::KeepAll{});
  service_ = rclcpp::Node::create_service<AxisState>(
    "request_axis_state", std::bind(&ODriveCanNode::service_callback, this, _1, _2),
    srv_qos.get_rmw_qos_profile());

  rclcpp::QoS srv_clear_errors_qos(rclcpp::KeepAll{});
  service_clear_errors_ = rclcpp::Node::create_service<Empty>(
    "clear_errors", std::bind(&ODriveCanNode::service_clear_errors_callback, this, _1, _2),
...
bool ODriveCanNode::init(EpollEventLoop * event_loop)
{
  ...
  if (!can_intf_.init(interface, event_loop, std::bind(&ODriveCanNode::recv_callback, this, _1))) {
    RCLCPP_ERROR(
      rclcpp::Node::get_logger(), "Failed to initialize socket can interface: %s",
      interface.c_str());
    return false;
  }
  ...
  if (!srv_clear_errors_evt_.init(
        event_loop, std::bind(&ODriveCanNode::request_clear_errors_callback, this))) {
...
void ODriveCanNode::service_callback(
  const std::shared_ptr<AxisState::Request> request, std::shared_ptr<AxisState::Response> response)
{
  {
  ...
  fresh_heartbeat_.wait(guard, [this, &call_time]() {
    bool complete =
      (this->ctrl_stat_.procedure_result != 1) &&  // make sure procedure_result is not busy
      (std::chrono::steady_clock::now() - call_time >=
       std::chrono::seconds(1));  // wait for minimum one second
    return complete;
...
void ODriveCanNode::service_clear_errors_callback(
  const std::shared_ptr<Empty::Request> request, std::shared_ptr<Empty::Response> response)
{
...
void ODriveCanNode::ctrl_msg_callback()
{
  ...
  switch (control_mode) {
    case ControlMode::kVoltageControl: {
      RCLCPP_ERROR(
        rclcpp::Node::get_logger(), "Voltage Control Mode (0) is not currently supported");
      return;
    }
...
inline bool ODriveCanNode::verify_length(const std::string & name, uint8_t expected, uint8_t length)
{
  ...
  if (!valid)
    RCLCPP_WARN(
      rclcpp::Node::get_logger(), "Incorrect %s frame length: %d != %d", name.c_str(), length,
      expected);
  return valid;
}
