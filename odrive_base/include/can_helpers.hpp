...
template <typename T>
T can_get_signal_raw(
  const uint8_t * buf, const size_t startBit, const size_t length, const bool isIntel)
{
  constexpr int N = 64;
...
template <typename T>
void can_set_signal_raw(
  uint8_t * buf, const T val, const size_t startBit, const size_t length, const bool isIntel)
{
  constexpr int N = 8;
...
constexpr void can_set_signal_raw(
  uint8_t * buf, const float val, const size_t startBit, const size_t length, const bool isIntel,
  const float factor, const float offset)
{
...
constexpr T can_get_signal(
  const TMsg & msg, const size_t startBit, const size_t length, const bool isIntel)
{
  return can_get_signal_raw<T>(can_msg_get_payload(msg).data(), startBit, length, isIntel);
}

template <typename T, typename TMsg>
constexpr void can_set_signal(
  TMsg & msg, const T val, const size_t startBit, const size_t length, const bool isIntel)
{
