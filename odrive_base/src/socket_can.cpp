...
bool SocketCanIntf::init(
  const std::string & interface, EpollEventLoop * event_loop, FrameProcessor frame_processor)
{
  ...
  if (!event_loop_->register_event(
        &socket_evt_id_, socket_id_, EPOLLIN, [this](uint32_t mask) { on_socket_event(mask); })) {
